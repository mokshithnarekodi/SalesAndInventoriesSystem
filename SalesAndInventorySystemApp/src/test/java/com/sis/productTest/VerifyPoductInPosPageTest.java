...
public class VerifyPoductInPosPageTest extends BaseClass {
	@Test(groups = "system")
	public void verifyPoductInPosPageTest() throws IOException, Throwable {
		...
		hp.getPOSLink().click();
		ThreadLocalUtility.getTest().log(Status.INFO, "verifying newly added product in POS page");
		...
		Thread.sleep(1000);
		boolean productVerifcation = driver.findElement(By.xpath("//h6[text()='"+ productname +"']")).isDisplayed();
		Thread.sleep(1000);
		wlib.waitForElementPresent(driver, driver.findElement(By.xpath("//h6[text()='"+ productname +"']")));
		Assert.assertTrue(productVerifcation);
		ThreadLocalUtility.getTest().log(Status.PASS, "product verified in pos page");
